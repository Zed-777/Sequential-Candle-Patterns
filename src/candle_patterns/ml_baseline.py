...
class PatternMLModel:
    ...
    def engineer_features(
        self, df: pd.DataFrame, patterns: List[Dict]
        ...
        df["volume_ma"] = (
            df["volume"].rolling(5).mean() if "volume" in df.columns else 1.0
        )
        ...
        for idx, row in df.iterrows():
            pattern_names = [
                p["pattern"] for p in patterns if p["timestamp"] == row["timestamp"]
            ]
    ...
    def train(
        ...
        logger.info(
            f"Training complete. Accuracy: {self.metrics['accuracy']:.3f}, ROC-AUC: {self.metrics['roc_auc']:.3f}"
        )

        return self.metrics

    def cross_validate(
        self, X: pd.DataFrame, y: Union[pd.Series, pd.DataFrame], n_splits: int = 5
    ) -> Dict:
        """
