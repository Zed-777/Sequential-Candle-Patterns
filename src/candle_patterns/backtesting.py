...
class BacktestEngine:
    ...
    def calculate_returns(
        self, df: pd.DataFrame, pattern_indices: List[int], hold_periods: int = 5
    ) -> pd.DataFrame:
        """
    ...
    def backtest_pattern(
        ...
        logger.info(
            f"Backtest {pattern_name}: {len(trades)} trades, {win_rate:.1%} win rate, Sharpe: {sharpe:.2f}"
        )

        return results

    def backtest_all_patterns(
        self, df: pd.DataFrame, pattern_data: List[Dict], hold_periods: int = 5
    ) -> pd.DataFrame:
        """
        ...
        for pattern_name in pattern_names:
            results = self.backtest_pattern(
                df, pattern_name, pattern_data, hold_periods
            )
    ...
    def calculate_equity_curve(
        self, trades: pd.DataFrame, initial_capital: float = 10000.0
    ) -> pd.Series:
        """
...
def evaluate_pattern_profitability(
    df: pd.DataFrame, patterns: List[Dict], hold_periods: int = 5
) -> Dict:
    """
    ...
    if total_trades > 0:
        ...
        evaluation["overall_win_rate"] = (
            (pattern_results["n_wins"].sum() / total_trades) if total_trades > 0 else 0
        )

    logger.info(
        f"Evaluation complete: {total_trades} total trades, {profitable_patterns}/{len(pattern_results)} patterns profitable"
    )
