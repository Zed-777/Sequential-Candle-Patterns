...
- Tests: unit + integration tests pass locally (19 passed).
- Security: addressed bandit low-severity findings by refining exception handling and explicit #nosec where appropriate.
- Maintenance: added `storage.cleanup_old_uploads()` with a CLI entry and scheduled daily cleanup workflow (`.github/workflows/cleanup.yml`) to enforce a 30-day retention by default.
