...
class TestCLITrain:
    ...
    def test_train_produces_model(self, tmp_path):
        ...
        result = runner.invoke(
            app, ["train", SAMPLE_CSV, "--model-out", str(model_out)]
        )
    ...
    def test_train_shows_metrics(self):
        result = runner.invoke(
            app, ["train", SAMPLE_CSV, "--model-out", "artifacts/_test_model.pkl"]
        )
        assert result.exit_code == 0, result.output
...
class TestCLIPredict:
    ...
    def _train_model(self, tmp_path):
        ...
        result = runner.invoke(
            app, ["train", SAMPLE_CSV, "--model-out", str(self.model_path)]
        )
    ...
    def test_predict_writes_csv(self, tmp_path):
        ...
        result = runner.invoke(
            app,
            ["predict", SAMPLE_CSV, "--model", str(self.model_path), "--out", str(out)],
        )
        assert result.exit_code == 0, result.output
    ...
    def test_predict_missing_model(self, tmp_path):
        result = runner.invoke(
            app, ["predict", SAMPLE_CSV, "--model", str(tmp_path / "nope.pkl")]
        )
        assert result.exit_code != 0
...
class TestCLIBacktest:
    ...
    def test_backtest_custom_hold(self, tmp_path):
        ...
        result = runner.invoke(
            app, ["backtest", SAMPLE_CSV, "--hold", "3", "--out", str(out)]
        )
        assert result.exit_code == 0, result.output
