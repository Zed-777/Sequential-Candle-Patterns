...
class TestDarkCloudCover:
    def test_dark_cloud_cover_detected(self):
        ...
        df = df_from_candles([
            make_candle("2024-01-01", 10.0, 15.0, 9.0, 14.0),   # Green: open=10, close=14
            make_candle("2024-01-02", 15.0, 16.0, 10.5, 11.0),  # Red: open=15 > 14, close=11 < midpoint(12)
        ])
        results = detect_patterns(df)
        assert any(r["pattern"] == "dark_cloud_cover" for r in results), \
    ...
    def test_dark_cloud_cover_not_detected_close_above_midpoint(self):
        ...
        df = df_from_candles([
            make_candle("2024-01-01", 10.0, 15.0, 9.0, 14.0),   # Green: open=10, close=14, mid=12
            make_candle("2024-01-02", 15.0, 16.0, 12.5, 13.0),  # Red: close=13 > midpoint(12)
        ])
        results = detect_patterns(df)
        assert not any(r["pattern"] == "dark_cloud_cover" for r in results), \
...
class TestBullishHarami:
    def test_bullish_harami_detected(self):
        ...
        df = df_from_candles([
            make_candle("2024-01-01", 20.0, 21.0, 9.0, 10.0),   # Large red: o=20, h=21, l=9, c=10
            make_candle("2024-01-02", 12.0, 18.0, 11.0, 16.0),  # Small green inside: h=18<21, l=11>9
        ])
        results = detect_patterns(df)
        assert any(r["pattern"] == "bullish_harami" for r in results), \
    ...
    def test_bullish_harami_not_detected_outside_range(self):
        ...
        df = df_from_candles([
            ...
            make_candle("2024-01-02", 12.0, 22.0, 11.0, 16.0),  # Green but h=22 > 21
        ])
        results = detect_patterns(df)
        assert not any(r["pattern"] == "bullish_harami" for r in results), \
...
class TestBearishHarami:
    def test_bearish_harami_detected(self):
        ...
        df = df_from_candles([
            make_candle("2024-01-01", 10.0, 21.0, 9.0, 20.0),   # Large green: o=10, h=21, l=9, c=20
            make_candle("2024-01-02", 18.0, 19.0, 11.0, 12.0),  # Small red inside: h=19<21, l=11>9
        ])
        results = detect_patterns(df)
        assert any(r["pattern"] == "bearish_harami" for r in results), \
    ...
    def test_bearish_harami_not_detected_outside_range(self):
        ...
        results = detect_patterns(df)
        assert not any(r["pattern"] == "bearish_harami" for r in results), \
...
class TestOnNeckLine:
    def test_on_neck_line_detected(self):
        ...
        df = df_from_candles([
            ...
            make_candle("2024-01-02", 16.5, 17.0, 14.8, 15.0),  # Red: close=15.0, near prior low=15.0
        ])
        results = detect_patterns(df)
        assert any(r["pattern"] == "on_neck_line" for r in results), \
...
class TestInNeckLine:
    def test_in_neck_line_detected(self):
        ...
        df = df_from_candles([
            ...
            make_candle("2024-01-02", 17.0, 18.0, 15.0, 16.0),  # Red: o=17<20, c=16>15
        ])
        results = detect_patterns(df)
        assert any(r["pattern"] == "in_neck_line" for r in results), \
